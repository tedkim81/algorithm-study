 ...
 * 그 와중에도 날치기 의결을 할 수 있으므로, 어떤 순간에도 방안에는 
 * 어느 한 정당이 과반수가 되지 않아야 한다.
 *
 * (input)
 * T: 테스트케이스의 수
 * N: 정당의 수
 ...
 * 가장 많은 정당에서 2명 나가거나, 가장 많은 정당에서 1명과 그 다음 정당에서 1명이 나가거나, 
 * 가장 많은 정당에서 1명만 나가는 경우 중 하나 선택.
 ...
 *
 * (solution 2)
 * 최다수 정당에서 일단 1명은 기본적으로 나가는거고, 
 * 최다수 정당에서 1명 뺀 상태에서 그다음 정당이 과반수가 된다면 
 * 거기에서 1명을 빼도록 한다. 그리고 나머지에 대해서 재귀적 호출.
 *
 * (solution 2 result)
 * 성공!!
 */
