 ...
 * - "가능한지"는 |자 또는 ㄱ자 모양이 되는지 인데,
 *   |자는 인접한 ㄱ자 2개를 모두 만족하는 경우이므로, ㄱ자만 고려해도 된다.
 * 슈도코드(?)를 작성해 보자.
 * --------------------------------------------
 ...
 * ptn에 false인 애들 탐색하면서 ㄱ자 모양이 되면 true
 ...
 *
 * (solution 2)
 * 최초의 grid에 선을 그려놓고 grid를 확장시켰다고 가정해보자.
 * 최초 grid의 어떤 패턴이 확장된 grid에 포함된다면,
 * - 선들 사이에 쏙 들어갔거나,
 * - 가로선에 걸쳤거나,
 * - 세로선에 걸쳤거나,
 * - 가로선과 세로선 모두에 걸쳤거나,
 * 이렇게 넷 중 하나일 것이다.
 * 즉, solution 1 에서의 "|자 또는 ㄱ자 모양이 되는지"를 찾는 문제는
 * "사분면으로 구분하는" 문제와 같은 것이다.
 * 문제를 다시 정의해보면,
 ...
 * (solution 2 result)
 * 실패.. WA(Wrong Answer).
 * 뭐가 잘못된걸까 열심히 찾아보다가 quad의 문제를 발견했다.
 * 16가지 모든 경우의 수를 찾아도 어차피 없는 사분면구성은 
 * 패턴의 크기도 작을거라고 가정했는데, 그게 틀렸다.
 *
 * (solution 3)
 * 16가지 사분면 구성 중에 최초 grid에 존재하는 구성만 남기는 코드를 추가
 *
 * (solution 3 result)
 * 성공!!
 */
