 ...
 * 나무 심을 영역은 1x1 영역에 구덩이 하나씩이 파져야 하는데, 이것을 두더지한테 시킨단다;;
 * 근데 두더지가 말귀를 그렇게 잘 알아듣는 애는 아니어서, "(10,10)을 파!"라고 시키면 
 * 그것을 중심으로 3x3영역, 즉 (9,9)~(11,11) 중에 하나를 지맘대로 골라서 판단다.
 * 최대 1000번을 시키게 되는데, 그 안에 직사각형 영역이 만들어져야 한다.
 * 두더지한테 땅파라고 지시할 위치들을 interactive하게 응답하자.
 *
 * (input)
 * T: 테스트케이스의 수
 * A: 20 또는 200
 ...
 * - int bat[9][26]; 로 하면 bat의 모든 값이 0으로 초기화 되는줄 알고 있었다;;
 */
				...
				for (int c=2; c<max_c; c++) {
					...
					if (bat[r][c] == 0) {
						...
						for (int p=0; p<8; p++) {
							int pr = r + peer_r[p];
							int pc = c + peer_c[p];
							if (pr >= 1 && pr <= max_r && pc >= 1 && pc <= max_c && bat[pr][pc] == 0) {
								out = true;
								break;
							}
						}
