 ...
 * 2019.1.8
 *
 * 한 모서리의 길이가 1km인 정육면체 형태의 UFO가 토론토 위에 떴단다.(얘들 요즘 문제가 다 인런식인듯..;;)
 * x,y,z축이 있는 3차원 공간 안에 있는데, 위아래를 나타내는 축은 y축이다.
 * UFO는 (0,0,0)을 중심으로 위치하고 있고, 토론토 땅바닥은 y=-3이면서 x와 z축으로 이루어진 평면에 평행한 평면이다.
 ...
 * 출력값 자체의 오차 및 크기 A에 대한 오차는 플러스마이너스 백분의 일이다.
 *
 * (solution 1)
 * x축 또는 z축 중에 아무거나 하나를 골라서(여기서는 x축으로 하자), 
 * 45도 회전시키면 그림자의 한변은 1로 유지, 다른 한변은 루트2가 되고, 이때가 x축 기준으로만 돌리는 조건에서는 최대값이 된다.
 ...
 * 그렇게 x축과 z축을 몇도 돌리는지 계산하고, 면의 중심점들을 그만큼 돌려서 좌표를 구한다. (이거 완전 수학문젠데..?)
 * Test set 1: 
 *   x축 기준 0도일때 넓이 1, 45도일때 넓이 1.414213. 순증가이다. 이분법으로 100번정도 반복하여 회전각을 구하자.
 *   이때는 그림자가 사각형이라 넓이 계산이 편하다.
 * Test set 2: 
 *   z축 기준 0도일때 넓이 1.414213, 45도일때 넓이 1.732050. 순증가이다. 이분법으로 100번정도 반복하여 회전각을 구하자.
 *   육각형이라 넓이 계산이 어렵긴 하지만, 높이가 1.414213으로 일정하고 좌우대칭이라는 특징을 이용하면 
 *   결국은 이것도 사각형 넓이 구하는 것과 같은 방식이 된다. (좌측 삼각형을 띄어서 우측에 붙이면 사각형이 된다.)
 ...
 *   평면과 이루는 각도는 45도( atan(1) )이다. 45도를 시작으로 0도까지 회전시키면 그림자의 넓이는 점점 커지고,
 *   0도일때 최대값인 루트2(1.414213..)가 된다. 이 과정을 이분법으로 구현하여 넓이가 A에 가장 가까울때의 각도를 구한다.
 ...
 *   이 각을 시작으로 0도까지 회전시키면 그림자의 넓이는 점점 커지고, 0도일때 최대값인 루트3(1.732050..)이 된다.
 *   이 과정을 이분법으로 구현하여 넓이가 A에 가장 가까울때의 각도를 구한다. 회전각은 (시작각 - 그 각도)
 ...
 * x축이나 z축으로 돌리는게 아니라, x축과 z축 중간에(45도) 있는 축으로 회전시키면 그림자를 정육각형까지 만들 수 있다.
 * 즉, 그 축을 중심으로 몇도 돌리면 되는지를 이분법으로 구하면, Test set 1과 Test set 2를 구분할 필요 없이
 * 답을 구할 수 있다.
 */
...
double get_area(double max_rad, double curr_rad, bool is_ex) {
	if (is_ex) {
		double len_1 = cos(max_rad - curr_rad);
		double len_2 = sqrt(3.0) * cos(curr_rad);
		return sqrt(2.0) * (len_1 + len_2) / 2;
	} else {
		return sqrt(2.0) * cos(curr_rad);
	}
}
...
string solve(double A) {
	...
	if (A <= sqrt(2.0)) {
		...
		max_rad = atan(sqrt(2.0)) + 0.0000001;
		hi_rad = max_rad;
		lo_rad = max_rad - acos(sqrt(2.0)/sqrt(3.0)) - 0.0000001;
		is_ex = true;
		for (int i=1; i<3; i++) {
			tmp1 = rotate_axis1(pts[i][2], pts[i][1], radian(45));
			tmp2 = rotate_axis2(pts[i][2], pts[i][1], radian(45));
			pts[i][2] = tmp1;
			pts[i][1] = tmp2;
		}
	}
	...
	for (int i=0; i<100; i++) {
		...
		area = get_area(max_rad, curr_rad, is_ex);
		if (area < A) {
			hi_rad = curr_rad;
		} else {
			lo_rad = curr_rad;
	...
	if (is_ex) {
		for (int i=0; i<3; i++) {
			tmp1 = rotate_axis1(pts[i][1], pts[i][0], diff_rad);
			tmp2 = rotate_axis2(pts[i][1], pts[i][0], diff_rad);
			pts[i][1] = tmp1;
			pts[i][0] = tmp2;
		}
	} else {
		for (int i=1; i<3; i++) {
			tmp1 = rotate_axis1(pts[i][2], pts[i][1], diff_rad);
			tmp2 = rotate_axis2(pts[i][2], pts[i][1], diff_rad);
			pts[i][2] = tmp1;
			pts[i][1] = tmp2;
		}
	}
