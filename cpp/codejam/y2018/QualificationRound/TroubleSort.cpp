 ...
 * 2018.12.28
 *
 * 어떤 알고리즘 연구소에서 버블소트에 영감을 받아 색다른(그리고 바보같은) 소팅 알고리즘을 개발했다.
 * 앞에서부터 3개씩을 확인하여 가장 왼쪽값과 가장 오른쪽값을 비교해서 왼쪽값이 더 크면 자리를 바꾸는 방식이다.
 * 숫자의 개수가 N이고 위치를 i라고 하면, i는 0부터 N-3까지 탐색하면서 i번째 값이 i+2번째 값보다 크면 바꾸는 것이다.
 * 그렇게 한바퀴 돌고 끝나는게 아니라, 한바퀴 돌때 값의 자리바꿈이 한번도 일어나지 않았으면 종료하고, 그렇지 않으면 계속 돌린다.
 ...
 * 따라서 짝수번째에 있는 숫자들 따로 정렬하고, 홀수번째에 있는 숫자들 따로 정렬한 후, (C++ 라이브러리를 믿는다..)
 * 각각에 대해서 동일한 위치에 있는 숫자를 비교해서 홀수번째의 숫자가 짝수번째의 숫자보다 큰 경우가 있다면, 
 * 걔가 답이다!
 * 여기서 주의해야할 점 몇가지,
...
string solve(int N, int V[]) {
	...
	for (int i=0; i<size_odd; i++) {
		// cout << v_even[i] << ", " << v_odd[i] << ", " << v_even[i+1] << endl;
		if (v_even[i] > v_odd[i]) {
			wrong_idx = i*2;
			break;
		}
