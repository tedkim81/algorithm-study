 ...
 * 2019.2.27
 *
 * 어떤 프로그래밍 언어가 최고인지 가려내기 위해서, N명의 사람들에게 가장 좋아하는 언어 하나씩을 말하도록 한다.
 * 설문을 마치고 나면, 어떤 언어 몇명, 또 어떤 언어 몇명 이런 식으로 정리가 될텐데, 이걸 언어별로 백분율로 기록하려고 한다.
 * 그런데, 백분율로 기록할때 소수점 첫째자리에서 반올림을 하여 자연수로 기록하려고 한단다.(얘는 통계쪽이랑 안맞는듯..)
 * 이러면 당연히 각 언어별 백분율값들을 다 더했을때 100이 되지 않는 경우가 발생할 수 있을 것이다.
 * 현재 설문이 진행중인 상황, 몇명이 아직 설문에 응답을 안한 상태에서, 지금까지 각 언어별로 몇명이 좋다고 했는지를 알고 있다.
 * 나머지 사람들의 최애 언어를 모두 응답 받을때, 선택 받은 모든 언어의 백분율값을 모두 합한 값이 가질 수 있는 최대값은 얼마인가?
 ...
 * 이미 선택을 받은 L개의 언어, 또는 아직 선택받지 않은 언어에, 최소한의 선택을 추가하여 소수점 아래가 0.5 이상이 되도록 해야 한다.
 * L개의 언어를 탐색하여 각각에 얼마를 더해야 반올림이 되는지를 기록하고, 선택받지 않았던 경우에는 얼마를 더해야 반올림이 되는지 구한다.
 * 더해야 하는 수가 작은 것부터 순서대로 더하면서 nn 만큼을 더한다.
 * 마지막에 남은 수는 무시해도 된다. 어차피 분모가 모두 같아서, 더해지는 수에 대한 소수점 증가분은 모두 같기 때문에,
 * L+1 개의 경우에 대해, 이미 반영했다면 더 반영할 필요가 없고, 모자라서 안된 거라면 다른 어디서도 안될 것이기 때문이다.
 * 자, 이제 슈도코드를 작성해 보자.
 * --------------------------
 * nn = N - (C[L]의 합)
