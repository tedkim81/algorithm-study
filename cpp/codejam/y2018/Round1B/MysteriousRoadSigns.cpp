 ...
 * 표지판들은 양면에 숫자가 적혀 있는데, 한면은 서쪽방향을 보고 있고(서쪽에서 동쪽으로 이동할때 보이는 방향, A1,A2,...),
 * 반대편은 당연히 동쪽방향을 보고 있다(B1,B2,...).
 * 이 숫자가 뭘까 생각을 해보니, A는 그 위치에서 동쪽으로(진행방향) A만큼 떨어진 곳에 마을이 있고,
 * B는 그 위치에서 서쪽으로(반대방향) B만큼 떨어진 곳에 마을이 있을 것이라는 가설을 세웠단다. (누가? 문제 만든 친구가)
 * 가설을 검증하기 위해, 전체 표지판 집합에서 아래 조건을 만족하는 부분집합을 구해보려는데, 그 크기가 가장 클 경우 얼마인지,
 * 그리고 그 최대 크기의 부분 집합은 몇개가 되는지를 구해보자.
 * 조건은,
 * - 부분집합은 연속된 표지판의 집합이다.(1,2,3,4,5 에서 2,3,4는 연속이지만 2,3,5는 연속이 아니다.)
 * - D+A=M, D-B=N. M지점과 N지점에 마을이 있음을 가리키는 표지판들의 (연속으로 나열된) 집합인데,
 *   A와 B 중 하나 이상만 맞는 값이면 된단다. (둘다 맞는 경우만 인정하도록 하면 문제가 너무 쉬위질테니까..ㅎㅎ)
 ...
 * 그 부분집합을 안다고 치고, S1부터 순서대로 생각해 보자. S1은 그 부분집합에 포함이 되어 있을 수도 있고, 아닐 수도 있다.
 * S1을 포함 했다치고, S2부터 시작하는 부분집합의 크기를 구해보자.
 * S1을 포함 안했다치고, S2에서 SS 구간에서 가장 큰 부분집합 크기를 구해보자.
 * 그리고, 위 둘 중 큰놈이 winner가 된다. 
 * 최대크기 부분집합의 수는, 위의 탐색을 하면서 제일 큰놈 나왔을때 이전 큰놈과 같다면 +1, 더 크다면 리셋해서 다시 1.
 ...
 * 부분문제화 하고 재귀적으로 함수를 호출하면서, 메모이제이션을 하려면 함수가 인자로 받은 값을 key로 하여 저장할 수 있어야 하는데,
 * "Si가 이전까지의 부분집합에 들어갈 수 있나?"를 구하려면 이전까지 만들어진 부분집합에 대한 정보가 있어야 하고, 
 * 이게 key로 쓰기에는 너무 복잡한 것이었던 것이었다.. 
 * 이번에도 너무 많은 시간을 사용했으므로, ANALYSIS를 컨닝하기로 결정..ㅠ.ㅠ
 * ANALYSIS를 읽어보니, 두가지 해법을 제시하고 있다.
 * 
 * 하나는 분할정복 알고리즘.
 * 중심의 S 하나를 기준으로 중심의 S를 포함하지 않고 그 좌측과 우측을 분할하여 재귀적으로 정복하는 방식이다.
 * 최대크기 부분집합은 좌측영역 안에 있을 수도 있고, 우측영역 안에 있을 수도 있고, 아니면 중심S를 포함하여 중심에 걸쳐져 있을 수 있다.
 * 따라서, 좌측과 우측 분할영역은 재귀적으로 탐색하고, 중심S가 포함된 부분집합을 찾아서 셋 중에 제일 큰게 winner가 되는 방식!
 ...
 * 불일치하는 동쪽의 S와 서쪽의 S에 대한 N값을 N1, N2라고 하면, (M, N1), (M, N2)에 대한 부분집합 중 큰걸 고르면 된다.
 * 그리고 중심S에 대해 N이 맞는 값이라고 가정하고 유사하게 한번더 찾아서 제일 큰 걸 찾아내면 된다.
 * 
 * ANALYSIS가 제시한 두번째 해법은 시간복잡도가 O(S) 이다.
 * (ANALYSIS에 나와 있는 그림 참고). 
 * 모든 S에 대해 M과 N을 미리 계산하여 저장해두고 얘들을 탐색한다.(이는 분할정복에도 적용 가능)
 * i번째 S를 탐색중이라고 할때, M-candidate은 M이 맞다고 할때 N과 start와 xstart가 나오고,
 * N-candidate은 N이 맞다고 할때 M과 start와 xstart가 나온다. 
 * 그러면 xstart-start+1 이 부분집합의 크기가 되고, M-candidate과 N-candidate 두쪽중 큰걸 고르면 되는 방식이다.
 * 여기에서, S들을 탐색하면서 값들을 연결하고, start와 xstart를 갱신하는 방식을 이해하는 것이 중요한데,
 * 글로 써놓기가 좀 애매하다.. 
 * 간단하게 요약하면, M가 N 값들을 체인처럼 연결하고 적절히 끊어가면서 가장 긴 체인을 남기는 방식이라 할 수 있다.
 * M-candidate은 M값 기준으로 이전의 두 체인(M-candidate체인과 N-candidate체인) 중 하나를 선택하는데,
 * 같은 체인(M-candidate 체인)의 M값과 같으면 걔랑 연결하고, 그렇지 않으면 반대 체인(N-candidate 체인)과 연결한다.
 * N-candidate도 유사한 방식으로 이어간다. 그리고 체인을 연결할때 만약 M을 연결하는데 연결되는 체인에 다른 M이 붙어 있으면
 * 걔 밑으로는 다 잘라버린다.
 * 
 * 위의 두개의 해법 중 분할정복 알고리즘을 이용하여 문제를 풀어보자.
 * 분할정복 알고리즘을 사용하는 것은 수학문제를 풀때 일반적인 공식을 이용하여 푸는 느낌이고, 
 * 두번째 해법은 해당 문제에 특화되어 특별하게 발견된 느낌이 든다.
 * 문제 출제자는 분할정복 알고리즘을 생각하고 문제를 냈는데, 누군가가 "이렇게 하면 더 빨라요!"라고 해법을 제시했고,
 ...
 *   update_max_size_info(left_size)
 *   update_max_size_info(right_size)
 *   update_max_size_info(mid_size)
 *   return max_size
 * ---------------------------------
 *
 * (solution 1 result)
