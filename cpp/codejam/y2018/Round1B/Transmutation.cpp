 ...
 *
 * 금속 중에 납을 제일 좋아하는 왕이, 금속을 막 바꿀 수 있는 연금술사에게, 
 * 최대한 많은 납을 만들어내라고 요구했다.
 * 총 M 종류의 금속이 있고, 각 금속은 다른 금속 두가지를 1그램씩 써서 
 * 1그램을 만들 수 있는 공식을 가지고 있다.
 * 현재 M 종류의 금속이 각각 몇 그램씩 있는지 알고 있고, 이 중 첫번째는 납이다.
 * 연금술사가 마음껏 지지고 볶아서 최선을 다해 납을 만들어 낸다고 할때, 
 * 납은 최대 몇그램이 될 수 있나? 
 * #이분법
 *
 * (input)
 * T: 테스트케이스의 수
 * M: 금속의 종류의 수
 * R1 R2 (M행): i번째 행에 대하여, G[i]를 1그램 만들기 위해 
 *             G[R1[i]] 1그램과 G[R2[i]] 1그램이 필요하다.
 ...
 * (solution 1)
 * 납을 1그램씩 차근차근 만들어보자. 
 * 납을 만들기 위해 필요한 두가지 금속의 G값을 확인해서 둘다 0보다 크면 G[1]++ 하고,
 * 두 금속은 G[R1[1]]-- , G[R2[1]]-- 한다. 
 * (R1,R2가 1부터 시작하므로 배열의 시작도 1로 시작하도록 저장하자.)
 * G[R1[1]] 또는 G[R2[1]]가 0이면 
 * 해당 금속을 만들기 위해 필요한 두 금속을 탐색해서 만들 수 있으면 만든다.
 ...
 * 알고리즘 자체의 문제인지 확인해 보기 위해 
 * cnt를 1 또는 0이 되도록 제한하여 테스트해 봤지만 마찬가지였다.
 *
 * (solution 2)
 * Test set 1만 깨자는 생각으로 solution 1의 최초 알고리즘으로 최대한 심플하게 다시 생각해 보자.
 * solution 1 알고리즘은 상단의 종료 조건과 중간에 R1, R2에 대해 
 * can_trans 호출할때 상호 간섭이 일어나는 문제에 있어서 뭔가 찜찜함이 있었다. 
 * 여기를 다시 생각해 보자.
 * ----------------------------
 * bool can_trans(int gg): // gg는 금속번호. gg를 만들수 있는지 여부 true or false 리턴.
 *   if G[gg] == -1: // 무한반복을 막기 위한 장치.
 *     return false
 *   if G[gg] > 0: // A 하나를 써서 A 하나를 만든다.
 *     G[gg]--
 *     return true
 *   tmp = G[gg]
 *   G[gg] = -1
 ...
 * print result
 * ----------------------------
 * 뭐야.. 이렇게 바꾸니까 Test set 1과 2를 그냥 통과하고, 
 * Test set 3는 당연히 TLE(Time Limit Error)가 발생했다.
 ...
 * R1 2개, R2 3개 만들었으면, gg를 2개 만들 수 있는건데, 
 * 그러면 남는 R2 1개는? 얘가 문제인거다..
 ...
 * 동일한 금속을 두번 탐색하게 되면 aa 이상의 납을 구할 수 없는거다.
 * aa를 포함하는 범위의 최소값과 최대값을 구해서 이분법으로 구현하면 답이 나올듯!
 * ----------------------------
 ...
 *
 * (solution 2 result)
 * 실패.. Test set 1과 2는 통과하는데, 
 * Test set 3에서 WA(Wrong Answer)가 뜬다. 성공할줄 알았는데..ㅠ.ㅠ
 * ANALYSIS를 보면 뭔가 맞게 접근한 것 같기는 한데, 
 * 설명이 자세하게 나와있지 않아서 내가 뭘 놓쳤는지를 못찾겠다.
