 ...
 * 2019.2.1
 *
 * 로봇들이 파티를 준비하기 위해 슈퍼마켓에서 비트를 사는 상황이다.(아.. 문제가.. 갈수록 가관이다..;;)
 * 쇼핑중인 로봇의 수는 R, 살 수 있는 전체 비트의 수는 B, 결제를 위해 대기중인 캐쉬어의 수는 C
 ...
 * 2. C는 R보다 크거나 같다. 즉, 로봇은 다른 로봇을 기다릴 일이 없이 바로 캐쉬어를 선택할 수 있다.
 *
 * 성능 고려 없는 단순한 알고리즘을 생각해 보면,
 * 1. 비트를 하나씩 할당하는데, 분배후 처리시간이 최소가 되게 해주는 캐쉬어에게 할당한다.(할당받은 캐쉬어 수 <= R)
 ...
 * 기본적인 전략은 같은데, 이분법을 통해 답을 구하는 과정에서, 나의 solution 1이 너무 복잡했고,
