 ...
 * 현재 쿠키 반죽들을 오븐에 넣기 전에 가장자리의 총 길이가 원하는만큼이 되도록 만들고 싶단다.(과하다 과해..)
 ...
 * (solution 1 result)
 * 실패! Wrong Answer 란다..
 * 생각해보니, solution 1 대로라면, 잘린 쿠키들 중 마지막 쿠키(잘린 것 중 제일 작은거)를 제외하고는 모두 대각선으로(d2로) 잘린게 된다.
 ...
 *
 * (solution 2)
 * 만약, 크기가 다른 쿠키 A,B,C를 모두 자른다고 할때, 쿠키를 잘라서 추가되는 가장자리 길이의 최소값은 (A,B,C 짧은면 길이 합)이고, 
 * 최대값은 (A,B,C 대각선 길이 합)이 된다. 그리고, 저 최소값과 최대값 사이의 모든 값은 A,B,C를 적당히 잘라서 만들어낼 수 있다.
 * (이게 중요한 부분이었다!) 따라서, 전체 쿠키들 중 어떤 쿠키들을 자를지 정하면 절단작업 후 pp에 가장 가까운 수는 쉽게 구할 수 있는 것이다.
 * (최소값 <= pp <= 최대값이면, 답은 P가 되고, pp > 최대값이면, 답은 P-(pp-최대값)이 된다.)
 * 자, 이제 어떤 쿠키들을 자를지만 정하면 되는데, 기본적으로는 각 쿠키마다 자를지 말지를 가정하여 깊이우선탐색을 하고, 최선의 값을 찾으면 된다.
 * 그러나, 쿠키가 100개인 경우 2의 100제곱만큼을 탐색해야 되는데 이는 너무 큰 수이므로, 동적프로그래밍 또는 가지치기를 통해 탐색을 줄여야 한다.
 * 슈도코드를 간단하게 써보자.
 ...
 * 그리고 쿠키를 잘라서 추가되는 길이에 대한 "구간"을 만들어서 pp를 포함하거나 가장 가까운 구간을 찾아서 답을 구하는 방식은 맞는 방식이다.
 * 다만 어떤 쿠키를 자를지에 대해 모든 경우를 탐색하는 것은, 직관적이기는 하지만 실행 불가능하다.
 * 그래서, 모든 쿠키를 탐색하고 각 쿠키를 자를 경우와 자르지 않을 경우를 고려하되, 경우의 수가 기하급수적으로 증가하지 않는 방법을 찾아야 한다.
 * 쿠키를 하나씩 탐색하면서 자를 경우와 자르지 않을 경우에 대해 "구간들"을 만들어서, 가능한 구간들의 집합을 구한다면?
 * 그 집합안에 있는 구간 내의 숫자는, 쿠키들을 잘라서(어떤 쿠키들을 잘랐는지는 알 수 없지만) 만들어 낼 수 있는 숫자라는 걸 보장할 수 있다.
