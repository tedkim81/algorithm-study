 ...
 * 
 * 아! 한놈씩 선택할지 말지를 판단하는 방식은 아무래도 어려워 보인다. 그렇다면, 일단 다 선택을 해서 위에서부터 거꾸로 만들어가다가
 * 더 이어갈 수 없을때, 뺄놈 빼고 더할놈 더해서(여기는 좀더 고민 필요) 다이어트를 시킨 후 다시 이어가도록 하면 어떨까?
 * 특정 시점에 [투입된 개미들],[남은 개미들] 두 집단이 있을때, 투입된 개미들의 구체적인 구성은 중요하지 않다.
 * 투입된 개미들의 전체 무게가 얼마냐가 중요하다. 그 전체무게를 wsum이라 할때, 남은 개미 중 다음 개미의 무게가 wsum/6 이상이라면
 * 투입 가능한 것이고, 투입 불가하다면, "투입된 개미들" 중에서 "다음 개미"보다 무거운 개미를 빼고 "다음 개미"를 투입시킬 수 있다면
 ...
 * 당연히 Test set 2의 풀이는 Test set 1도 커버한다. Test set 1 풀이가 Test set 2 풀이에 비해 좀더 직관적이라 생각해내기 쉽다.
 * Test set 1 풀이와 Test set 2 풀이 각각에 대하여 코딩을 각각 작성했고, 설명은 각 코드에 주석으로 작성했다.
 * 둘 모두, 문제를 부분문제화 해서 함수를 재귀적으로 작성하고, 동적 프로그래밍(DP. 메모이제이션 사용)을 사용하는데,
 ...
 * nn은 0번째 개미부터 nn번째 개미까지가 이 함수의 후보가 됨을 의미하고, mm은 후보 개미들 중 대상 개미들의 무게 합의 최대값을 의미한다.
 * 이렇게 nn과 mm이 주어졌을때, 대상 개미들은 최대 몇마리가 될 수 있는가를 return 해야 한다.
 * 그러면, solve(N-1, 충분히 큰값) 을 호출하면, 답(전체 개미를 후보로 했을때 대상 개미수의 최대값)을 return 해준다.
 * 
 * mm을 "최대값"이라는 모호한 개념으로 지정한 이유는, 맨 아래 개미의 위에 있는 개미들의 무게합이 맨 아래 개미 무게의 6배 이하가 
 * 되어야 한다는 문제의 조건 때문에, 맨 아래 개미를 제외하면서 부분문제화 할 때 "남은 애들은 이 무게를 넘으면 안된다"는 조건이
 * 생기기 때문이다.
 *
 * 후보 개미들 중 맨 아래 개미가 "정답 집합"에 포함이 될까 안될까를 각각 가정하여 부분문제를 재귀적으로 호출하고 최대값을 구하는 방식이다.
 * nn은 0~99 이고, mm은 0~100000 이고, nn*mm은 1000만보다 작은 수이므로 메모이제이션이 가능하다.
 */
// #include <stdio.h>
// #include <iostream>
// #include <algorithm>
 ...
 * mm자리에 무게가 들어가면 Test set 2에서는 너무 큰값이 되어 버리기 때문에 DP를 쓸 수 없기 때문.
 * nn은, 1번째부터 nn번째까지가 후보가 됨을 의미하고, (0번째가 아니라 1번째부터 시작. mm과 비교할 수 있도록 하기 위해.)
 * mm은, 후보들 중 대상이 몇마리가 되어야 할지를 지정하는 값일때,
 * solve는, 대상 개미들의 무게합의 최소값을 return 한다.
 *
 * Test set 1 풀이와 유사하게, 맨 아래 개미가 "정답 집합"에 포함이 될까 안될까를 각각 가정하여 
 * 부분문제를 재귀적으로 호출하는 방식인데, 여기서 중요한 차이점은, solve의 return값이 답이 되는 것이 아니라,
 * mm에 들어갈 수 있는 최대값이 답이 된다는 것이다. 
 * mm에 답보다 큰수가 들어온 경우, mm마리의 개미로 탑을 쌓는 것이 불가능하므로 solve는 -1을 리턴한다.
 * 즉, 답이 될 수 있는 최대값은 139니까, mm에 139부터 차례로 시도하여 solve가 -1이 아닌 값을 리턴할때의 mm값이 
 * 답이 되는 것이다.
 */
#include <stdio.h>
#include <iostream>
